    ...
    <div className="bg-primaryBackground h-full flex flex-col justify-center items-center relative">
      <div className="w-full h-5/6 flex justify-center items-center">
        {images.map((image, index) => (
          ...
          >
            <div className='w-4/6 absolute onboarding_text '>
              <h3 className='text-2xl font-bold text-center'>{text[index]}</h3>
            </div>
            <img src={image} alt={`Image ${index + 1}`} className="object-cover mx-auto" />
          </div>
        ))}
      </div>
      <div className="w-full h-2/6 flex justify-center items-start  mt-12 pt-12">
        {images.map((_, index) => (
          <span
            key={index}
