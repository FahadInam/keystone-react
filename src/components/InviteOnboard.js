...
function SetNewPassword() { 
    ...
    const {values, errors ,touched ,handleBlur, handleChange, handleSubmit} = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            console.log(data)
            try {
                const response = await axios.post(`http://192.168.18.43:8000/api/v1/invitations/${inviteID}/${token}`, data);
                const { firstname, lastname } = response.data.data;
                store.dispatch({ type: 'SET_FIRST_NAME', firstname });
                store.dispatch({ type: 'SET_LAST_NAME', lastname });
                navigate('/dashboard');
